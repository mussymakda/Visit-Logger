# QR Code Visit Logger - Copilot Instructions

## Project Overview
This is a Laravel 12 + Filament 4 application for managing sponsors with QR codes and tracking interior designer visits using mobile devices.

## Architecture & Data Flow
- **Backend**: Laravel 12 with SQLite database (configured via Laravel Boost MCP tools)
- **Admin Panel**: Filament 4 at `/admin` - sponsor/user management, visit analytics
- **Designer Panel**: Filament 4 at `/designer` - QR scanner dashboard, visit history
- **Mobile Workflow**: HTML5 QR scanner → sponsor verification → photo capture → API submission
- **QR Generation**: Online service (qrserver.com API) to avoid GD extension dependency
...
This app uses Laravel Boost MCP server providing specialized tools:
- `mcp_laravel-boost_application-info` - Get package versions & models
...
│   ├── Users/          # Admin users only (role filter)
│   ├── InteriorDesigners/  # Interior designer users
│   └── Sponsors/       # Sponsor CRUD with QR management
└── Designer/Pages/     # Designer panel pages
    └── Dashboard.php   # QR scanner interface
```

### Database Schema Patterns
- **users**: `role` enum ('admin'|'interior_designer') determines panel access
...
- Form components moved to `Schemas/` subdirectories 
- Resources use separate schema classes: `SponsorForm.php`, `SponsorsTable.php`
...
- `php artisan make:filament-resource` - Create admin panel resources
